...
print(sys.argv[14])
network_properties = json.loads(sys.argv[14].replace("'", "\""))

shuffle = sys.argv[15]
cluster = sys.argv[16]

#number of retrainings per setting
n_retrain = 10
seeds = list()
seeds.append(42)
...
for retrain in range(len(seeds)):
    folder_path_retraining = os.path.join(folder_path, "Retrain_" + str(retrain))
    arguments = list()
