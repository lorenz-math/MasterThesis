...
class DefineDataset:
    def __init__(self,
        ...
        #self.n_samples = self.n_collocation + 2 * self.n_boundary * (1 + (self.input_dimensions - 1)) + self.n_internal + self.n_object
        self.n_samples = self.n_collocation + self.n_boundary * ( 2* (self.input_dimensions - 1)) + self.n_internal + self.n_object
        self.BC = None
    ...
    def assemble_dataset(self):
        ...
        fraction_boundary = int(self.batches * 2 * (self.input_dimensions - 1) * self.n_boundary / self.n_samples)
        ...
        if self.extrema_values is not None:
            ...
            #generation of collocation points in the domain
            ...
            #generation of training points on the boundary
            x_list_b = list()
            y_list_b = list()
            ...
            for i in range(self.time_dimensions, self.time_dimensions + self.space_dimensions):
                ...
                #generation of training points on the boundary via the midpoint rule
                if self.type_of_coll == 'quad':
                    x_boundary_0 = self.generator_points(self.n_boundary, self.space_dimensions, self.random_seed)
                    zero = torch.zeros(self.n_boundary,1)
                ...
                if self.type_of_coll == "quad":
                    x_boundary_1 = self.generator_points(self.n_boundary, self.space_dimensions, self.random_seed)
                    one = torch.full((self.n_boundary,1), 1.)
            ...
            #generation of training points for initial data
            if self.type_of_coll == "quad":
                x_time_0 = self.generator_points(self.n_initial, self.space_dimensions, self.random_seed)
            ...
            if self.parameters_values is not None:
                ...
                y_time_0=torch.ones_like(x_time_wo_i)
        ...
        #if self.extrema values is None (which is not the case for the Semilinear wave equation)
        else:
    ...
    def transform_param_data(self, tensor):
        if self.type_point_param == "normal":
    ...
    #different ways to compute training points
    def generator_points(self, samples, dim, random_seed, n_time_step=None):
        ...
        #implementation of midpoint rule (main part that was added for the master's thesis in this file)
        elif self.type_of_coll == "quad":
